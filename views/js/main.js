...
function updatePositions(scrollTop) {
  ...
  /*
     According to https://developer.chrome.com/devtools/docs/demos/too-much-layout
     if we update the layout and then read the value of a layout property we cause a forced
     update, since the browser needs to provide a consistent value for the asked value.

     So, if we update the 'left' style property of each image, and then in the following iteration
     of the loop we access the body.scrolltop property, we will cause a layout update at every cycle.

     We can solve it easily by pulling out the value (which does not depend on loop variables) and
     then making a css transform on the x axis 
  */
  ...
  for (var i = 0; i < items.length ; i++) {
    ...
    var left =   100*phase;
  /*
    We use translate3d since it seems that the browser in this case aggregates all the elements
    in a layer composited by the GPU.

    The alternative would have been using a big canvas as background image, but it seems to work
    this way as well.
  */
    //We account for differnt browsers
    items[i].style.WebkitTransform="translate3d("+ left +"px,0px,0px)";
    items[i].style.MozTransform="translate3d("+ left +"px,0px,0px)";
    items[i].style.transform="translate3d("+ left +"px,0px,0px)";
  }
...
//Sync update with requestAnimationFrame
window.addEventListener('scroll', function()  {
    var scrollValue = document.body.scrollTop;
    window.requestAnimationFrame(function () {
        updatePositions(scrollValue)
    });
});

//Creates pizzas of the mover class
//We call it not only on load, but also on viewport resize
var bakePizzas = function() {
    //Flushes existing items
    var mover = document.getElementsByClassName('mover');
    while (mover[0]){
        mover[0].parentNode.removeChild(mover[0]);
    }
    ...
    for (var i = 0; i < 200; i++) {
        ...
        //Created a dedicated image so that scaling is not needed
        elem.src = "images/pizza_small.png";
        elem.style.height = "100px";
        elem.style.width = "70px";
        elem.basicLeft = (i % cols) * s;
        elem.basicTop= (Math.floor(i / cols) * s) ;
        //Set the 'left' style position, since we won't update it directly
        //in the update loop, but we will be usign a transform
        elem.style.position= 'fixed';
        elem.style.left = elem.basicLeft + 'px';
        elem.style.top =  elem.basicTop + 'px';
        //We draw only the elements visible in the viewport
        if (elem.basicLeft < document.documentElement.clientWidth+s) {
            if (elem.basicTop< document.documentElement.clientHeight+s) {
...
//Since  the we draw only the pizzas visible in the viewport, we regenerate
//them when it is resized.
window.onresize = bakePizzas;

// Generates the sliding pizzas when the page loads.
document.addEventListener('DOMContentLoaded',bakePizzas);
